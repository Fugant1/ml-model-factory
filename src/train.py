...
def _train_model(X_train, y_train, preprocessor, model_name):
    '''This function will just train the model based on the train sample provided'''
    model_config = MODELS[model_name]
    model_object = model_config['model']
    model_params_to_grid = model_config.get('params', {})

    if isinstance(model_object, (XGBRegressor, XGBClassifier)):
        #no preprocessing needed for XGBoost
        pipeline = Pipeline(steps=[("model", model_object)])
    else:
    ...
    #sets the scoring based on the model
    scoring = "accuracy"
    if isinstance(model_object, XGBRegressor):
